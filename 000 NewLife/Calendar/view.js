...
if (!(await _nlVerify())) {
	dv.el("div", "", {cls: "newlife-feature-locked"}).innerHTML = `
		<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 20px;text-align:center;color:var(--text-muted);">
			...
			<h3 style="margin:0 0 8px 0;color:var(--text-normal);">任务日历需要激活 NewLife</h3>
			<p style="margin:0 0 16px 0;max-width:400px;">请在设置中输入 License Key 激活完整功能。</p>
			<p style="font-size:0.85em;">命令面板 → NewLife: 激活 NewLife</p>
		</div>`;
	return false;
}
// ═══════════════════════════════════════════════════════════════
